...
<div class="container" style="max-width: 1200px; padding-top: 1.5rem; padding-bottom: 3rem;">
    ...
    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 2rem;">
        ...
        <div style="display: flex; gap: 0.75rem;">
            ...
            <div style="padding: 0.65rem 1.25rem; border-radius: var(--radius-sm); background: var(--surface-2); color: #10b981; font-weight: 600; font-size: 0.82rem; display: flex; align-items: center; gap: 0.5rem; border: 1px solid var(--border);">
    ...
    <div class="wellness-banner">
        <div style="max-width: 700px; position: relative; z-index: 1;">
            ...
            <div style="display: flex; gap: 0.75rem;">
                <button onclick="location.href='mindfulness.php'" style="padding: 0.65rem 1.5rem; border-radius: var(--radius-sm); background: var(--surface-solid); color: var(--primary-dark); border: none; font-weight: 600; cursor: pointer; font-size: 0.85rem; transition: var(--transition);">Practice Now</button>
                <button onclick="location.href='mental_health_resources.php'" style="padding: 0.65rem 1.5rem; border-radius: var(--radius-sm); background: rgba(255,255,255,0.1); color: white; border: 1px solid rgba(255,255,255,0.2); font-weight: 600; cursor: pointer; font-size: 0.85rem; backdrop-filter: blur(8px);">Resources</button>
            </div>
        </div>
    </div>
    ...
    <div style="display: grid; grid-template-columns: repeat(4, 1fr); gap: 1rem; margin-bottom: 2rem;">
        ...
        <div class="stat-card" style="background: <?php echo($latest_score['risk_level'] === 'Critical' || $latest_score['risk_level'] === 'High') ? 'rgba(239, 68, 68, 0.1)' : 'rgba(16, 185, 129, 0.1)'; ?>; border-color: <?php echo($latest_score['risk_level'] === 'Critical' || $latest_score['risk_level'] === 'High') ? 'rgba(239, 68, 68, 0.2)' : 'rgba(16, 185, 129, 0.2)'; ?>;">
            <div style="font-size: 0.72rem; font-weight: 600; color: <?php echo($latest_score['risk_level'] === 'Critical' || $latest_score['risk_level'] === 'High') ? '#ef4444' : '#10b981'; ?>; text-transform: uppercase; margin-bottom: 0.75rem; letter-spacing: 0.04em;">Risk Level</div>
            <div style="font-size: 1.5rem; font-weight: 700; color: <?php echo($latest_score['risk_level'] === 'Critical' || $latest_score['risk_level'] === 'High') ? '#ef4444' : '#10b981'; ?>;"><?php echo $latest_score['risk_level']; ?></div>
        </div>
        <?php
endif; ?>
    </div>
    ...
    <div class="quick-actions">
        <a href="take_assessment.php" class="quick-action">
            <div class="quick-action-icon" style="background: var(--surface-2); color: var(--primary);">📋</div>
        ...
        <a href="student_ai_chat.php" class="quick-action">
            <div class="quick-action-icon" style="background: var(--surface-2); color: #10b981;">✨</div>
        ...
        <a href="mood_journal.php" class="quick-action">
            <div class="quick-action-icon" style="background: var(--surface-2); color: #f59e0b;">📔</div>
        ...
        <a href="student_appointments.php" class="quick-action">
            <div class="quick-action-icon" style="background: var(--surface-2); color: #3b82f6;">📅</div>
    ...
    <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 1.5rem; margin-bottom: 2rem;">
        <!-- Assessment Chart -->
        <div style="background: var(--surface-solid); border-radius: var(--radius); border: 1px solid var(--border); padding: 2rem; box-shadow: var(--shadow-sm); height: 400px; display: flex; flex-direction: column;">
            <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 1.5rem; border-bottom: 1px solid var(--border); padding-bottom: 1rem;">
                <h2 style="font-family: 'Outfit', sans-serif; font-weight: 700; font-size: 1.15rem; margin: 0; color: var(--text);">Wellness Score Trend</h2>
        ...
        <div style="background: var(--surface-solid); border-radius: var(--radius); border: 1px solid var(--border); padding: 2rem; box-shadow: var(--shadow-sm); height: 400px; display: flex; flex-direction: column;">
            <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 1.5rem; border-bottom: 1px solid var(--border); padding-bottom: 1rem;">
                <h2 style="font-family: 'Outfit', sans-serif; font-weight: 700; font-size: 1.15rem; margin: 0; color: var(--text);">Emotional Mood Trend</h2>
...
Chart.defaults.color = getComputedStyle(document.documentElement).getPropertyValue('--text-dim').trim() || '#8b95a5';
Chart.defaults.borderColor = getComputedStyle(document.documentElement).getPropertyValue('--border').trim() || 'rgba(0,0,0,0.1)';
