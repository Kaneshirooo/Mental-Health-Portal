...
<head>
    ...
    <style>
        ...
        .science-card:hover { background: var(--surface-solid); border-color: var(--border-hover); transform: translateY(-3px); box-shadow: var(--shadow-sm); }
    </style>
</head>
<body>
...
<div class="container" style="max-width: 1000px; padding-top: 1.5rem; padding-bottom: 3rem;">
    ...
    <div id="ariaRecCard" style="display:none; background: var(--surface-2); border: 1px solid var(--border); border-radius: var(--radius); padding: 1.5rem 2rem; margin-bottom: 2rem; display: flex; gap: 1.5rem; align-items: center; box-shadow: var(--shadow-sm); animation: fade-up 0.6s ease-out;">
        <div style="width: 50px; height: 50px; border-radius: 14px; background: var(--surface-solid); display: flex; align-items: center; justify-content: center; font-size: 1.5rem; border: 1px solid var(--border); flex-shrink: 0;">🪄</div>
        <div style="flex: 1;">
            <h3 style="font-family: 'Outfit', sans-serif; font-size: 0.9rem; font-weight: 800; color: #a21caf; text-transform: uppercase; letter-spacing: 0.05em; margin-bottom: 0.25rem;">Aria's Daily Focus</h3>
            <p id="ariaRecText" style="color: #701a75; font-size: 0.92rem; line-height: 1.5; font-weight: 500; font-style: italic;">Seeking a recommendation based on your last mood log... ✨</p>
        </div>
    </div>
    ...
    <div style="background: var(--surface-solid); border-radius: var(--radius-lg); padding: 2.5rem; border: 1px solid var(--border); box-shadow: var(--shadow-sm); margin-top: 2.5rem;">
    ...
    <style>
        ...
        .mood-btn:hover { background: var(--surface-solid); border-color: var(--border-hover); }
        .mood-btn.selected { background: var(--surface-solid); border-color: var(--primary); color: var(--primary); box-shadow: var(--shadow-sm); }
    </style>
    ...
    <script>
        ...
        async function getAriaRecommendation() {
            const card = document.getElementById('ariaRecCard');
            const text = document.getElementById('ariaRecText');
            
            try {
                const res = await fetch('mindfulness_recommendation_api.php');
                const data = await res.json();
