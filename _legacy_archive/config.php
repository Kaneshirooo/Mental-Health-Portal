...
/**
 * generateCSRFToken() — Generates a unique token for forms.
 */
...
/**
 * verifyCSRFToken() — Validates the token from a POST request.
 */
...
try {
    ...
    // NEW: Chat History for AI Context
    ...
    // Add is_priority to appointments if missing (for head counselor priority cases)
    $cols = $conn->query("SHOW COLUMNS FROM appointments LIKE 'is_priority'");
    if ($cols && $cols->num_rows === 0) {
        $conn->query("ALTER TABLE appointments ADD COLUMN is_priority TINYINT(1) NOT NULL DEFAULT 0 AFTER counselor_message");
    }
}
...
/**
 * checkLoginBruteForce() — Checks if an IP is temporarily locked out.
 */
function checkLoginBruteForce($ip)
{
    ...
    return ($res['attempts'] >= 5);
}

/**
 * registerLoginAttempt() — Logs a failed login attempt.
 */
...
/**
 * sendOTP() — Sends a 6-digit verification code via PHPMailer.
 */
function sendOTP($email, $code)
{
    ...
    try {
        ...
        // Content
        $mail->isHTML(true);
        $mail->Subject = 'Your Verification Code — ' . SMTP_NAME;
        ...
        // In a real app, log this error: $mail->ErrorInfo
        return false;
    }
}
?>
