...
<div class="container" style="max-width: 900px; padding-top: 6rem; padding-bottom: 6rem;">
    ...
    foreach ($questions as $question):
        if ($question['category'] !== $category)
            continue;
?>
                <div class="question-strip" style="background: white; padding: 2.5rem 2rem; border-radius: var(--radius); border: 1px solid var(--border); box-shadow: var(--shadow-sm);">
...
?>
    ...
    endforeach; ?>
            ...
            <div style="margin-top: 3rem; display: flex; justify-content: space-between; align-items: center; background: var(--surface-2); padding: 1.5rem 2rem; border-radius: var(--radius); border: 1px solid var(--border);">
                <?php if ($stepIdx > 0): ?>
                    <button type="button" onclick="goStep(<?php echo $stepIdx - 1; ?>)" style="background: white; border: 1px solid var(--border); padding: 0.75rem 1.75rem; border-radius: var(--radius-sm); font-weight: 600; cursor: pointer; color: var(--text-muted); transition: var(--transition); font-size: 0.9rem;">← Back</button>
                <?php
    else: ?>
...
<div id="confirmModal" style="display:none; position:fixed; inset:0; z-index:99999; background:rgba(15,23,42,0.55); backdrop-filter:blur(8px); align-items:center; justify-content:center;">
    <div style="background:white; border-radius:var(--radius-lg); padding:2.5rem; max-width:400px; width:90%; text-align:center; box-shadow:var(--shadow-lg); animation:modalPop 0.25s cubic-bezier(0.34,1.56,0.64,1);">
